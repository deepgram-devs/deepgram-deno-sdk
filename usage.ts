import { querystring } from "./deps.ts";

import {
  UsageField,
  UsageFieldOptions,
  UsageOptions,
  UsageRequest,
  UsageRequestList,
  UsageRequestListOptions,
  UsageResponse,
} from "./deps.ts";

export class Usage {
  constructor(private _credentials: string, private _apiUrl: string) {}

  private apiPath = "/v1/projects";

  /**
   * Retrieves all requests associated with the provided projectId based
   * on the provided options
   * @param projectId Unique identifier of the project
   * @param options Additional filter options
   */
  async listRequests(
    projectId: string,
    options?: UsageRequestListOptions
  ): Promise<UsageRequestList> {
    const requestOptions = { ...{}, ...options };
    let response = null;
    try {
      response = await fetch(
        `https://${this._apiUrl}${
          this.apiPath
        }/${projectId}/requests?${querystring.stringify(requestOptions)}`,
        {
          method: "GET",
          headers: {
            Authorization: `token ${this._credentials}`,
            "Content-Type": "application/json",
            "X-DG-Agent": "deno-sdk/1.0.0",
          },
        }
      );
      if (response.ok) {
        return response.json();
      } else {
        throw `${response.status} ${response.statusText}`;
      }
    } catch (error) {
      throw "DG: Cannot List requests. " + error;
    }
  }

  /**
   * Retrieves a specific request associated with the provided projectId
   * @param projectId Unique identifier of the project
   * @param requestId Unique identifier for the request to retrieve
   */
  async getRequest(
    projectId: string,
    requestId: string
  ): Promise<UsageRequest> {
    let response = null;
    try {
      response = await fetch(
        `https://${this._apiUrl}${this.apiPath}/${projectId}/requests/${requestId}`,
        {
          method: "GET",
          headers: {
            Authorization: `token ${this._credentials}`,
            "Content-Type": "application/json",
            "X-DG-Agent": "deno-sdk/1.0.0",
          },
        }
      );
      if (response.ok) {
        return response.json();
      } else {
        throw `${response.status} ${response.statusText}`;
      }
    } catch (error) {
      throw "DG: Cannot fetch request. " + error;
    }
  }

  /**
   * Retrieves usage associated with the provided projectId based
   * on the provided options
   * @param projectId Unique identifier of the project
   * @param options Options to filter usage
   */
  async getUsage(
    projectId: string,
    options?: UsageOptions
  ): Promise<UsageResponse> {
    const requestOptions = { ...{}, ...options };
    let response = null;
    try {
      response = await fetch(
        `https://${this._apiUrl}${
          this.apiPath
        }/${projectId}/usage?${querystring.stringify(requestOptions)}`,
        {
          method: "GET",
          headers: {
            Authorization: `token ${this._credentials}`,
            "Content-Type": "application/json",
            "X-DG-Agent": "deno-sdk/1.0.0",
          },
        }
      );
      if (response.ok) {
        return response.json();
      } else {
        throw `${response.status} ${response.statusText}`;
      }
    } catch (error) {
      throw "DG: Cannot fetch usage. " + error;
    }
  }

  /**
   * Retrieves features used by the provided projectId based
   * on the provided options
   * @param projectId Unique identifier of the project
   * @param options Options to filter usage
   */
  async getFields(
    projectId: string,
    options?: UsageFieldOptions
  ): Promise<UsageField> {
    const requestOptions = { ...{}, ...options };
    let response = null;
    try {
      response = await fetch(
        `https://${this._apiUrl}${
          this.apiPath
        }/${projectId}/usage/fields?${querystring.stringify(requestOptions)}`,
        {
          method: "GET",
          headers: {
            Authorization: `token ${this._credentials}`,
            "Content-Type": "application/json",
            "X-DG-Agent": "deno-sdk/1.0.0",
          },
        }
      );
      if (response.ok) {
        return response.json();
      } else {
        throw `${response.status} ${response.statusText}`;
      }
    } catch (error) {
      throw "DG: Cannot fetch usage. " + error;
    }
  }
}
